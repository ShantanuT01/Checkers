...
public class CheckersServer extends Server
{
	private Queue<Integer> clientsWaitingForGame;
	private Map<Integer, Checkers> games;
    ...
    public CheckersServer(int port)
    {
    	...
    	games = new HashMap<Integer, Checkers>();
    ...
    public void process(int id, String message)
    {
    	...
    	Checkers game = games.get(id);
    	
    	//check if client is in a game and if that game is not over
    	if(game != null && !game.isGameOver())
    	{
    		//check if it is this players turn
    		Piece player = xClients.contains(id) ? Piece.WHITE : Piece.BLACK;
    		Piece curr = game.getCurrentPlayer();
    		if(curr == player)
    		{
     ...
     *		1. Create a new Checkers game for these clients
     *		2. Randomly assign each client X or O
     *		3. Assign clientId -> Checkers game in clientGames map
     */
	public void onJoin(int id)
	{
		...
		if(clientsWaitingForGame.size() >= 2)
		{
			...
			//create Checkers game and mapping from clients -> game
